...
export default function ContratanteHomePage() {
  ...
  // Busca todos os contratos do contratante autenticado
  const fetchContracts = async (authToken: string) => {
    try {
      ...
      if (!response.ok) {
        ...
        // Lança erro detalhado se a API não retornar 2xx
        throw new Error(
          errorData.message ||
  ...
  const handlePagar = async (contratoId: string) => {
    ...
    try {
      ...
      // Recarrega a lista de contratos para refletir o novo status
      await fetchContracts(token);
    } catch (error: any) {
  ...
  // Escolhe o ícone de status do contrato com base no status
  const getStatusIcon = (status: Contrato['status']) => {
    switch (status) {
      ...
      case 'CONCLUIDO':
        return '/icons/check-verde.svg'; // poderia ser outro ícone específico para concluído
      default:
        return '/icons/pendente.svg';
    }
  };

  // Função auxiliar para filtrar contratos por status
  ...
  // Renderiza o card de um único contrato
  ...
  return (
    <div className="min-h-screen bg-white">
      <header className="flex items-center justify-between px-8 py-4 border-b border-gray-200">
        <Link
          href="#"
          className="text-2xl font-bold text-blue-600 no-underline hover:text-blue-700 transition-colors"
        >
          SoundBridge
        </Link>
